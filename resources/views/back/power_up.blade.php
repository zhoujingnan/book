...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  ...
  <tr>
    ...
    <form action="{{url('/backpower/upDo')}}" enctype="multipart/form-data" method="post">
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
    	<input type="hidden" name="_token" value="{{csrf_token()}}">
    	<input type="hidden" name="p_id" value="{{$arr[0]['p_id']}}">
      	<tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        	<td align="right" valign="middle" class="borderright borderbottom bggray">	
        		权限名称：
        	</td>
        	<td align="left" valign="middle" class="borderright borderbottom main-for">
        		<input type="text" name="p_name" value="{{$arr[0]['p_name']}}" class="text-word">
        ...
        <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        	...
        	<td align="left" valign="middle" class="borderright borderbottom main-for">
        		<input type="text" name="cname" value="{{$arr[0]['cname']}}" class="text-word">
...
$(function(){
	...
	$("[name='p_name']").blur(function(){
		...
		if(p_name){
			...
			$(".s_name").html('<font color="red">请填写权限名</font>');
	...
	$("[name='cname']").blur(function(){
		var cname=$(this).val();
		if(cname){
			$(".c_name").html('<font color="green">√</font>');
			n=true;return true;
		}else{
			$(".c_name").html('<font color="red">请填写控制器名</font>');
